...
class SemanticTable: Table {
    ...
    func run() throws -> Int? {
        ...
        //it can (it's a transducer); otherwise, get the sponsor to do it.
...
class ReduceTable: Table, TableWithTransitionsWithIntKey {
    ...
    func run() throws -> Int? {
        ...
        if parser.newTree != nil {
            tree = parser.newTree as VirtualTree?
            parser.newTree = nil as VirtualTree?
        } else {
            // Capture that one subtree (if any) in the stacks
            debug("\tArray of size (\(parser.treeStack.count)) - [\(parser.left)~\(parser.right)]")
            let indices = parser.left...parser.right
            let treeStackWithNils = parser.treeStack[indices]
...
public final class Parser: Transducer {
    ...
    func buildTreeFromIndex(index: Int) {
